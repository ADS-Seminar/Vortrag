...
\begin{frame}{Laufzeiten bei binären Suchbäumen}
    ...
    \begin{table}
        ...
        \visible<6-> {$O(1)$} \\
        \visible<7-> {$S$ = Paste($S_1, S_2$)} & \only<7-> {$O(h_1)$} \\
...
% Rotation erhält Suchbaumeigenschaft
% Rotation ändert Prioritätenreihenfolge rot <-> blau (Rest bleibt gleich)
\begin{frame}{Find + Update}
    ...
    \begin{column}{.35\textwidth}<1->
        \raisebox{-\totalheight}{
            %\includegraphics<1>[width=.8\textwidth]{img/Impl_Find.pdf} %TODO: Bild fehlt noch
            \includegraphics<1>[width=.8\textwidth]{img/dummy.pdf}
            %\includegraphics<2>[width=.8\textwidth]{img/Impl_Insert.pdf} %TODO: Bild fehlt noch
            \includegraphics<2>[width=.8\textwidth]{img/dummy.pdf}
            %\includegraphics<3>[width=.8\textwidth]{img/Impl_Delete.pdf} %TODO: Bild fehlt noch
...
\begin{frame}{Mengenoperationen}
    ...
    \begin{column}{.35\textwidth}<1->
        \raisebox{-\totalheight}{
            ...
            \includegraphics<1>[width=.8\textwidth]{img/dummy.pdf}
            %\includegraphics<2>[width=.8\textwidth]{img/Impl_Paste.pdf} %TODO: Bild fehlt noch
            \includegraphics<2>[width=.8\textwidth]{img/dummy.pdf}
...
% TODO: Es reicht "Delete" zu analysieren
% TODO: Laufzeit v. Delete(x) = Find(x) + leftspine + right spine
\begin{frame}{Mulmuley Games}
    ...
    \end{itemize}
    \only<5->{$P, S$ total geordnet}

    \only<6->{$\forall p \in P, s \in S:\ p < s$}

\end{frame}
\begin{frame}{Mulmuley Games}
    Ziehe zufällige Elemente aus einer Menge $X$ bis ein Stopper gezogen wurde oder die Menge leer ist.
    % TODO: nach hinten: Zähle Anzahl der Spieler, die größer sind als alle vorherigen.
    \begin{itemize}
        \item Spiel A
        \begin{itemize}
            \item $X = P \cup B$
        ...
        % \end{itemize}
    \end{itemize}
    \only<+->{Zähle die Anzahl der Spieler, die größer sind als alle vorherigen und nach dem ersten Trigger gezogen wurden.} % TODO: Aufdeckzeiten anpassen (Spiel E weggelassen)
...
\begin{frame}{Laufzeitanalyse}
    ...
    \begin{itemize}
        ...
        \item<4-> Länge rechter Grat % TODO? gute Übersetzung?
        des linken Teilbaums +\\ Länge linker Grat des rechten Teilbaums
    \end{itemize}
    \includegraphics<2>[width=.8\textwidth]{img/dummy.pdf}% TODO: Graph mit markiertem Knoten (Schema)
    \includegraphics<3>[width=.8\textwidth]{img/dummy.pdf}% TODO: Zeichne Suchpfad ein
    \includegraphics<4>[width=.8\textwidth]{img/dummy.pdf}% TODO: Zeichne Grate ein
\end{frame}
\begin{frame}
...
\section*{Fazit}
\begin{frame}{Fazit}
    % Laufzeiten beim Treap
\end{frame}


% TODO: Es reicht "Delete" zu analysieren
% TODO: Laufzeit v. Delete(x) = Tiefe(x) + leftspine + right spine
%                                 |                 |
%                              Suchen            Rotieren
%                              Game A             Game D
%                              O(log n)           < 2
